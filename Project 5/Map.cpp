...
bool Map::IsSolid(glm::vec3 position, float* penetration_x, float* penetration_y)
{
	...
	*penetration_y = 0;
	...
	if (position.x < left_bound || position.x > right_bound) return false;	// if outside the map, you are touching nothing
	...
	if (tile_x < 0 || tile_x >= width) return false;	// tiles must be in bounds of tileset (no negative or larger than width/height)
	if (tile_y < 0 || tile_y >= height) return false;

	int tile = levelData[tile_y * width + tile_x];
	if (tile == 0) return false;	// 0 means empty
